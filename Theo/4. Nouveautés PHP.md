...
```
On voit que c'est une syntaxe abrégée. Le fait de recevoir des variables dans le constructeur et de mettre un corps vide { }, php sait qu'il doit directement créer les propriétés en fonction de ce qui a été reçu en paramètres.
...
```
Cool non ? :) 
##.2 L'affectation Null coalescent
...
```
Ce code va planter pour $country2: Fatal error: Uncaught Error: Call to a member function getCountry() on null.
...
$country2 = $customer2?->getAddress()?->getCountry();
```
Dans ce cas, $country prendra la valeur nulle dans deux cas:
- si \$customer est null
