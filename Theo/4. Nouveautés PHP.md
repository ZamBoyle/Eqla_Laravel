...
```
On voit que c'est une syntaxe abrégée. Le fait de recevoir des variables dans le constructeur en mettant leur visibilité (public, private, protected), php sait qu'il doit directement créer les propriétés en fonction de ce qui a été reçu en paramètres. On peut aussi avoir un mixe:
```php
class Customer {
    public int $age;
