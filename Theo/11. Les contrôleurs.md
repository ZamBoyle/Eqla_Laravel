...
Comme nous verrons plus tard l'utilisation des bases de données, nous utiliserons un tableau de users dans notre contrôleur qui contiendra tous nos users.
...
class PersonController extends Controller
{
    ...
    {
        $array = $this->Search('firstname', $firstname);
        return view('results', ['users' => $array]);
    }
