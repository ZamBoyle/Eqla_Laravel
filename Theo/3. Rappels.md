...
Un avis personnel, si vous êtes certain que vous allez avoir des classes enfants de votre classe, je vous conseille de mettre protected au lieu de private. Comme ça vous pourrez avoir accès à ces membres protected. En effet, si vous avez des propriétés/méthodes qui sont privées, il ne vous sera pas possible d'y accéder dans vos classe enfants.
...
### 2.5 Héritage
L'héritage c'est le fait de créer une classe en héritant des fonctions et propriétés de notre classe parente si elles sont qualifiées de public ou protected. 
