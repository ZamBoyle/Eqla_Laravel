...
Exemple 4: Passage d'une variable d'une autre portée(scope) à une fonction anonymé
Comme vous le savez une variable est visible dans sa portée. En anglais, on utilise le terme de 'Scope'. C'est notion de portée existe dans tous les langages.
    ...
    */
    ...
    $func2 = function ($str) {
        ...
        for ($x = 0; $x < strlen($str); $x++) {
            ...
            //Alors on ajoute la lettre courante en majuscule à $result
            //Sinon on ajoute la lettre courante en minuscule à $result
            $result .= ($modulo == 0) ? strtoupper($current) : strtolower($current);
        }
        return $result;
    };
...
C'est comme private, on ne pourra accéder accéder aux méthodes et attributs à l'extérieur de la classe. La différence ?? Tout sera accessible depuis la classe ET les classes dérivées. Si vous mettez un membre privé, il ne sera pas accessible depuis une classe enfant.
