...
PHP 7.0 fournit la possibilité d'utiliser un typage fort. Il est possible de typer les paramètres des fonctions et des méthodes (fonctions d'une classe) avec int, float, string, bool, array, etc. Il est possible aussi d'indiquer si la fonction retourne un type ou si elle n'en retourne pas (void).
Mais afin  de ne pas rompre la compatibilité d'anciens scripts, PHP 7.0 ne force pas le typage par défaut. Mais si on veut vraiment avoir un typage fort, il faut utiliser en haut de nos fichiers PHP la directive suivante: 
```php
