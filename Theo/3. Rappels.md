...
- [2. La programmation orientée object (POO)](#2-la-programmation-orientée-object-poo)
  - [2.1 Classe](#21-classe)
    ...
    - [2.1.4 L'encapsulation](#214-lencapsulation)
    - [2.1.5 Visibilité d'une méthode/attribut: public, private, protected](#215-visibilité-dune-méthodeattribut-public-private-protected)
  - [2.2 Objets / Instances](#22-objets--instances)
...
#### 2.1.5 Visibilité d'une méthode/attribut: public, private, protected
...
Alors pour l'écriture des membres privés d'une classe (méthodes/attributs), certains ont pris la convention d'écrire tout membre public avec un underscore. Pour l'exemple, j'ai appliqué cette convention. 
```php
public string $nom;
private bool $_contientLait;
```
...
C'est comme private, on ne pourra accéder accéder aux méthodes et propriétés à l'extérieur de la classe. La différence ?? Tout sera accessible depuis la classe ET les classes dérivées. Si vous mettez un membre privé, il ne sera pas accessible depuis une classe enfant.

Un avis personnel, si vous êtes certain que vous allez avoir des classes enfants de votre classe, je vous conseille de mettre protected au lieu de private. Comme ça vous pourrez avoir accès à ces membres protected. En effet, si vous avez des propriétés/méthodes qui sont privés, il ne vous sera pas possible d'y accéder dans vos classe enfants.

### 2.2 Objets / Instances
Un objet, c'est la "matérialisation" d'une classe. D'un type théorique, la classe, on la concrétise en faisant un objet.
On utilie le mot clef new() et à l'intérieur des parenthèses on fournit les éventuels paramètres nécessaires au constructeur de la classe.
```php
$cupcake1 = new Cupcake("Pinky", array("sucre" => "12 gr", "lait" => "10 ml"), true, true);
```
...
### 2.5 Héritage
L'héritage c'est le fait de créer une classe en héritant des fonctions et propriétés de notre classe parente.
On possède tous les avantages de notre parent et on peut ajouter/redéfinir des méthodes/propriétés.
...
class Cake extends Cupcake
{
    ...
    public function displayHauteur()
    {
        echo "La hauteur de $this->nom est de $this->hauteur cm.";
    }
...
$cake = new Cake("Pinky", 5.3, array("sucre" => "12 gr", "lait" => "10 ml"), true, true);
$cake->displayName();
...
Ce code affichera:
```
Ce cake s'appelle Pinky.
Liste des ingrédients:
sucre avec 12 gr.
lait avec 10 ml.
La hauteur de Pinky est de 5.3 cm.
```
Notre nouvelle classe Cake va hériter des méthodes et propriétés de la classe parente si celles-ci sont public ou protected.
### 2.6 Opérateurs de résolution de portée
Il permet d'accéder aux constantes et aux membres statiques.
Il s'écrit :: dans notre exemple pour accéder à la constante statique FOUR, on utilisera :: au lieu de ->
```php
$cupcake1 = new Cupcake("Pinky", array("sucre" => "12 gr", "lait" => "10 ml"), true, true);
if($cupcake1::FOUR){
    echo "Ce cupcake doit être cuit au four.";
}
```