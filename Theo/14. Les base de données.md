...
- [Les bases de données](#les-bases-de-données)
  ...
  - [5. Eloquent](#5-eloquent)
    ...
    - [5.9 whereBetween](#59-wherebetween)
...
On récupère aussi l'enregistrement et dans la méthode update on donne un tableau associatif où l'on donnera affectera les attributs de la classe.
```php
App\Models\User::find(1)->update(['name'=>'Pietta','email'=>'johnny.pietta@gmail.com']);
```

### 5.8 where()
Si on veut utiliser la clause where SQL, on peut le faire via la méthode where() que l'on va illustrer par différents exemples. La syntaxe pourra vous sembler étrange mais on s'y fait assez vite. Vous aurez plus d'infos dans la [documentation Laravel](https://laravel.com/docs/8.x/queries#basic-where-clauses).

Pour les prochains exemples on va réinitialier la base de données:
```console
php artisan migrate:fresh
```
Nous allons ajouter 5 enregistrements grâce à la méthode insert de la classe User:
```php
php artisan tinker
use App\Models\User;
User::insert([
...
L'opérateur de comparaison est le deuxième paramètre à fournir à where(). On peut utiliser les opérateurs suivants:
- \> et \<
- \>= et \<=
- \<\>
- like
...
```php
$user = User::where('email','=','johnny.pietta@gmail.com')->get();
```
...
Afficher les utilisateur dont l'email n'est pas 'johnny.piette@gmail.com'
